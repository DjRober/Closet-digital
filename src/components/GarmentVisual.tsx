...
export function GarmentVisual({
  ...
  // SVG representation for each clothing item
  const renderIconSvg = () => {
    const strokeColor = colorHex || '#44403c';
      ...
      case 'jacket':
        return (
          ...
          >
            ...
            <circle cx="36" cy="38" r="1.5" fill={strokeColor} />
            <circle cx="36" cy="46" r="1.5" fill={strokeColor} />
          </svg>
        );
  ...
  return (
    <div
      className={`relative flex items-center justify-center rounded-xl bg-stone-100/90 border border-stone-200/80 transition-colors ${className}`}
      style={{
        backgroundColor: colorHex ? `${colorHex}12` : '#f5f5f4',
      }}
    >
      {renderIconSvg()}
    </div>
